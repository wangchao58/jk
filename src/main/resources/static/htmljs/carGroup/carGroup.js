...
jQuery(function($) {
    ...
    jQuery(grid_selector).jqGrid({
        url: '/carGroup/selectByExample',
        datatype: "json",
        mtype: "post",
        colModel:[
            {label:'id',name:'tId', editable:false,hidden:true },
            {label:'群名',name:'tGroupName', editable: true },
            {label:'分类',name:'tSort', editable: true },
            {label:'群主',name:'tGroupLord',  editable: true},
            {label:'发布时间',name:'tCreateTime', editable: true},
            {label:'归属地',name:'tAttribution', editable: true},
            {label:'状态',name:'tStatus', editable: true},
    ...
    function operation(cellvalue, options, rowObject) {
        var htmlstr = "<div class='visible-md visible-lg hidden-sm hidden-xs btn-group'>"
            ...
            + rowObject.tId
            + "')><i class='icon-edit bigger-120'></i>审核通过</button></div>";
        return htmlstr;
    }
...
function deletes() {
    ...
    if(ids != null && ids != "") {
        layer.confirm('此操作不可逆,是否继续？', {
            ...
            $.ajax({
                url: '/carOwner/deleteByPrimaryKey',
                data: {'ids':row},
                type: "POST",
...
function openEditDiag(uuid) {
    //页面层
    $.ajax({
        url: '/carGroup/updateByPrimaryKeySelective',
        data:{"tId":uuid,"tStatus":1},
        type: "POST",
        async: true,
        dataType: "json",
        success: function (result) {
